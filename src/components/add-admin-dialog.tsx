...
type Props = SingleProps | GlobalProps;

export function AddAdminDialog({ facilityId, facilityName, facilities }: Props) {
  const [isOpen, setIsOpen] = useState(false);
  const isGlobal = !!facilities;

  const { isPending, error, handleSubmit } = useFormAction(hqCreateAdmin, () =>
    setIsOpen(false),
  );
          ...
          <Card className="w-full max-w-md bg-white rounded-3xl shadow-2xl border-none overflow-hidden animate-in zoom-in duration-200">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-sm text-slate-900 leading-tight">
                    {isGlobal ? "施設管理者を新規登録" : "施設管理者を登録"}
                  </h3>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
                    {isGlobal ? "法人共通・クイック登録" : facilityName}
                  </p>
                </div>
              </div>
              <Button
            ...
            <CardContent className="p-8">
              <form action={handleSubmit} className="space-y-6">
                {!isGlobal && <input type="hidden" name="facilityId" value={facilityId} />}
                  ...
                  {isGlobal && (
                    <div className="space-y-2 text-left">
                      ...
                      <select
                        ...
                        name="facilityId"
                        required
                        className="w-full h-12 rounded-2xl border border-slate-200 bg-slate-50/50 focus:bg-white transition-all text-sm font-bold px-4"
                      >
