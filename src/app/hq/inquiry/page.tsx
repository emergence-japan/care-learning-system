      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-2">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">本部サポート窓口</h1>
            <Badge variant="outline" className="bg-indigo-50 text-indigo-600 border-indigo-200 font-bold px-3 py-1 rounded-lg">
        ...
        {inquiries.length === 0 ? (
          ...
          inquiries.map((inquiry) => (
            <Link key={inquiry.id} href={`/hq/inquiry/${inquiry.id}`}>
              <Card className="group relative hover:ring-2 hover:ring-indigo-600/10 hover:border-indigo-600/30 transition-all duration-300 rounded-xl overflow-hidden border-slate-200 bg-white shadow-sm hover:shadow-md">
                <div className={`absolute left-0 top-0 bottom-0 w-1 ${
                  inquiry.status === 'UNREAD' ? 'bg-indigo-500' : 
                ...
                <CardContent className="p-4 pl-6 sm:p-5 sm:pl-7">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                      ...
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center border border-slate-100 group-hover:bg-indigo-600 group-hover:text-white group-hover:border-indigo-600 transition-all duration-300 text-slate-300">
...
function StatusBadge({ status }: { status: string }) {
  switch (status) {
    case "UNREAD":
      return <Badge className="bg-indigo-50 text-indigo-600 border-indigo-100 px-2 py-0 h-5 rounded-md text-[10px] font-bold">送信済み</Badge>;
    case "READ":
